...
task <- makeRegrTask(
  id = "defoliation-all-plots", data = data,
...
ps_xgboost <- makeParamSet(
  ...
  makeDiscreteVectorParam("fw.basal.methods",
    len = 8,
    values = c(
      "carscore", "cforest.importance", "FSelector_chi.squared", "FSelectorRcpp_gain.ratio",
...
tune_wrapper_svm <- makeTuneWrapper(lrn_ksvm,
  resampling = inner,
  par.set = ps_svm_chi.squared,
...
tune_wrapper_xgboost <- makeTuneWrapper(lrn_xgboost,
  resampling = inner,
  par.set = ps_xgboost_chi.squared,
...
library(parallelMap)
parallelStart(
  mode = "multicore", cpus = 30, level = "mlrMBO.feval",
  mc.set.seed = TRUE
)
