...
Overall, the support vector machine (SVM) outperformed other algorithms such as random forest (RF), extreme gradient boosting (XGBoost), lasso (L1) and ridge (L2) regression by at least three percentage points.
...
Some individual features were removed before using the datasets for modelling when being numerically equivalent to another feature based on the pairwise correlation being greater than $1 - 10^{-10}$.
...
Additional variable selection was solely devoted to the respective feature selection methods in this study. <!-- FIXME: I don't understand this sentence. -->
...
Differences to other feature sets were very small for lasso (below one p.p.) and a bit larger for ridge regression (between two and five p.p.).
XGBoost showed very poor performances for some feature sets and fills the ten last places of the ranking (\autoref{tab:perf-worst-10}).
...
The use of filters with RF resulted in a substantial increase in performance in all tasks with the exception of NRI, in which the difference among all filters was also the smallest (\autoref{fig:filter-effects-no-filter}).
XGBoost's performance depended strongly on feature selection: In two out of six tasks, using no filter resulted in the worst or second worst performance.
XGBoost showed the highest overall differences between filters for a single task: for feature set HR, the range is up to 15 p.p. (\enquote{CMIM} vs. \enquote{no filter}) (\autoref{fig:filter-effects-no-filter}).
...
Overall, SVM used the least features across all plots and among all learners, the single highest fraction being 24\% for test set Laukiz2.
...
For HR, four features in the infrared region (920 nm - 1000 nm) were identified by the model to be most important, being assciated with a mean decrease in RMSE of around 1 p.p.
