...
The use of filters with RF resulted in a substantial increase in performance in all tasks with the exception of NRI, in which the difference between all filters was also the smallest (\autoref{fig:filter-effects-no-filter}).
...
% - never the best
The Borda filter achieved in only one instance (RF on HR-NRI-VI) the best performance of\ all filters (\autoref{fig:filter-effects-borda}).
...
Especially XGBoost had substantial problems when using PCA as a filter method and scored the four\ worst results (\autoref{tab:perf-worst-10}).
