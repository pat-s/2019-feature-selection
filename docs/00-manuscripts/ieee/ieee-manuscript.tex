...
\begin{abstract}
	...
	Filter methods show potential to be helpful in high-dimensional situations while still keeping the ability to interpret feature effects of fitted models, which is an essential constrain in environmental modeling studies.
...
This makes it possible to extend the benchmark comparison matrix of studies in a semi-automated way, possibly stumbling across unexpected findings of process settings that would never have been tested otherwise \cite{ma2015}.
...
It's final feature order is determined by the sum of all single filters ranks.
...
After feature rank correlations of $> 0.9$ betwforeen different bin sizes were observed in a side analysis, \texttt{\(n_{bin}\) = 10} was found to be a reasonable setting to go with.
...
SVM showed the best overall performance with a difference of around three percentage points to the next best model (RF) (\autoref{tab:best-learner-perf}).
...
Hence, care should be taken when predicting to the landscape scale using models fitted on this dataset due to their lack of generalizability caused by suboptimal distributed training data.
...
The upper range of 600 used for hyperparameter {\texttt{nrounds}} for model XGBoost in this study could be a potential limiting factor with respect to the performance of the datasets including NRI variables (\autoref{tab:hyperparameter_limits}).
