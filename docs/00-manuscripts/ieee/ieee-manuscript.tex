...
The airborne hyperspectral data was acquired during two flight campaigns which took at noon on September 28th and October 5th 2016.
...
% JM: I have no idea what the Borda ensemble filter is. Maybe add one more sentence on this specific filter
...
% JM: reading further on, it might be that there are only three classes, please refine, Patrick!
Filter methods can be grouped into four classes: Correlation-based, entropy-based, linear and non-linear methods.
...
The idea of splitting the features into different sets originated from the question whether feature-engineered indices from reflectance values have a positive effect on model performance, as for example demonstrated by \cite{pena2017} within a spectro-temporal setting.
% JM: just a little reminder to avoid sentences where the main verb appears long after the sentence has started
...
It may be recommendable to increase this upper limit to a value closer to the number of features in the dataset in order to be able to exploit the full range of this hyperparameter.

\subsubsection{Feature set differences}

One objective of ths study was whether expert-based or data-driven feature engineering has a positive influence on model performance.
