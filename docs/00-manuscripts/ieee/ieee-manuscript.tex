...
% single column figure to Gbe placed prior to an earlier double column
% figure.
...
\author{Patrick~Schratz,
	...
	\thanks{B.Bischl is head of the statistical learning and data science group at the Department of Statistics, Ludwig-Maximilian-University Munich.}% <-this % stops a space
...
\begin{abstract}
	Analyzing highly-correlated feature-rich datasets from hyperspectral remote sensing data is a challenging task.
	This study compared multiple machine-learning and statistical learning methods using filter-based feature-selection methods on a hyperspectral dataset.
	In the case study, defoliation at trees (\%) in northern Spain was modeled as a function of band reflectance.
	Multiple feature sets were compared using derived features (expert-based and data-driven).
	Among the selected methods, support vector machine (SVM) outperformed others such as random forest (RF), extreme gradient boosting (XGBoost), lasso (L1) and ridge (L2) regression by about three percentage points.
	The combination of feature sets showed small increases in predictive performance while no substantial differences between single feature sets were observed.
	Filter methods helped in certain task-learner-filter settings to achieve better predictive performances while ensemble filters did not have a substantial impact on performance.
	Depending on the dataset-learner combination, filter methods can help to improve predictive performance and lower computational runtime.
	Permutation-based feature importance estimated features around the red edge to be most important for the models.
	However, features in the near-infrared region (800 nm - 1000 nm) were also important to achieve acceptable performances.
	The dataset used in this work was suboptimal and more training data is needed for more generalizable conclusions.
\end{abstract}

% Note that keywords are not normally used for peer-review papers.
\begin{IEEEkeywords}
	hyperspectral imagery, forest health monitoring, machine learning, feature selection, model comparison, filter, imaging spectroscopy
\end{IEEEkeywords}
...
\noindent To use the hyperspectral data's full potential, all possible vegetation indices supported by the R package hsdar (89 in total) as well as all possible \ac{NRI} combinations were calculated from the reflectances.
...
\noindent To account for geometric offsets within the hyperspectral data, which were reported with up to 1 m from \ac{ICGC}, a buffer of two meters around the centroid of each tree was used when extracting the reflectance values.
...
To reduce the feature space of a dataset, conceptually differing approaches exist: wrapper methods, filters, penalization methods (lasso and ridge) or \ac{PCA} \cite{das2001, jolliffe2016}.
Since this study focuses mainly on filter methods, only this sub-group will be introduced in greater detail in the following subsections.
...
The combination of both penalities is called 'elastic net' but was not used in this work.
...
Hence, after having calculated all pair-wise correlations between features, for pairs which exceeded the threshold of 0.9999999999, the feature with the largest mean absolute correlation was removed from the dataset.
...
Hyperparameters were tuned using \ac{MBO} within a nested spatial \ac{CV} \cite{mlrmbo, schratz2019}.
...
To optimize the number of features used for model fitting, the percentage of features was added as a hyperparameter during the optimization stage.
...
Permutation feature importance calculates the loss of predictive performance for each feature by permuting it in a random manner during prediction.
...
However, ALE plots need visual inspection and as the number of features increases, making objective comparisons/conclusions becomes challenging.

In this work we applied permutation-based feature importance and \ac{ALE} plots (HR only) to estimate feature importance.
...
We used the best-performing algorithm on the HR task (i.e. SVM) to calculate the feature importance.
...
Regression with ridge penalty (L2) showed a high variance when comparing results across tasks: In four out of six tasks (all which VI variables and HR-NRI) the error was enourmous (\autoref{tab:perf-worst-10}).
...
This one outlier caused the error estimates of these folds and the average estimate across all folds to be offscale.
...
Regarding the problematic predictions of the rigde learner, a careful inspection of the hyperparameter optimization procedure of the fitted models did not reveal any apparent implementation mistakes.
...
Ultimatevily, the results of (\autoref{tab:tune-perc-sel-features}) should be taken with care as they rely on single model-filter combinations and are subject to random variation.
...
Even though this selection included a large number of available indices, some possibly relevant indices might have been missed by relying on the preselection of indices offered by the package.
...
ALE plots can be a powerful tool for interpreting feature importances of correlated features but might be limited in interpretability if the absolute feature values are only semi-meaningful.
...
We hope that this work and its methodology raises awarness about the application of filter methods to tackle high-dimensional problems in the environmental modeling field.
