...
Defoliation was measured via visual inspection using 5\% intervals.
...
A \ac{PCA} was done to analyse the correlation within the variables of the different feature sets (FIXME figref).
...
The following NDVI-based formula was used for the NRI calculation:
...
The choice of NDVI serving as the base formula for NRI calculation was determined by the implementation of the \texttt{nri} function in the \texttt{hsdar} R package.
...
Inspecting the results of the \ac{PCA} for all feature sets, HR showed a high correlation among variables with a \ac{POV} of 84\% in the first \ac{PC} and 14\% in the second \ac{PC}, summing up to a cumulative \ac{POV} of 98\% (\autoref{fig:pca-pov}).

Feature sets VI (51\% PC1, 25\% PC2, 7\% PC3) and NRI (51\% PC1, 25\% PC2, 8\% PC3) showed a more diverse distribution across their respective \ac{PC}s.
...
The following information was provided externally by the company which was in charge for the image acquisition and preprocessing of the data.
