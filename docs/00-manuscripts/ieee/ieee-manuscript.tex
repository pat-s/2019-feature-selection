...
While for RF least features were selected when Oiartzun was the test set ($n = 2$), most features were used for this plot during model fitting for XGBoost and SVM (\autoref{tab:tune-perc-sel-features}).
Most features were selected during optimization when training without Laukiz2 or Oiartzun and least when Laukiz1 served as the test set.
RF used only two features when Oiartzun served as the test data (0.0028 \% of 7675) while 34 and 230 features were seen as optimal during optimization when Laukiz1 and Laukiz2 were the test data, respectively.
...
Also, to represent a tree with a crown diameter of around two meters accordingly, more than one square meter (= one pixel) is needed.
...
The applied buffer only included a pixel value if the distance to the centroid of a pixel was smaller than the buffer radius.
