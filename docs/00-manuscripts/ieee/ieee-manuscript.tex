...
For the Lasso and Ridge models however the internal non-spatial CV approach due to promising results in previous applications.
...
Methods like \ac{PDP} do not produce reliable estimates in such situations because they create unrealistic situations between covariates.
...
Nevertheless, recent methods like \ac{ALE} plots aim to tackle this problem by utilizing marginal distributions of features over a small window to focus on the effect of a single feature.
However, 
...
We used the algorithm which showed the best performance for the HR task to calculate the feature importance.
...
To balance the effort/gain ratio and to be able to relate the results to wavelength regions, we decided to estimate feature importance only for the dataset with the raw reflectance values (HR).
...
% FIXME: explain in methods why only 2 datasets are shown
...
For both datasets most features among the ten most important ones cluster around a wavelength range of 700 nm - 750 nm.
For feature set HR four features in the infrared region (920 nm  - 1000 nm) were identified by the model to be important due to causing a mean RMSE decrease of around one percentage point.
Overall, most features show a small importance only with average decreases in RMSE below 0.5.
...
\subsection{Predictive Performance}
The best aggregated performance of this study (27.98 RMSE) has to be seen in the light of model overfitting (see \autoref{subsec:perf-plot-char}).
When leaving out Laukiz2 during aggregation, the mean RMSE would be 19.
Taking into account the overfitting, more training data representing the health stages of a plot is needed.
A model can only make robust predictions if it has learned relationships which span the whole range of the response variable.
Hence, the aggregated mean RMSE of 27 here should not be used to make statements about the trained model of this study.
In contrast, by inspecting the fold level performances, one can say that the model does a decent job predicting defoliation ranging from 50\% to 100\% but fails for 0\% - 50\%.
