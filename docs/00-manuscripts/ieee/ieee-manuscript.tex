...
\noindent The decision to use a buffer of 2 m for the extraction of an observation's reflectance value was a complex decision.
...
By using a buffer of 2 meters (or more), the probability of including information from other trees into the final value increases, blurring the actual value of the tree observation.
...
In our view using a buffer of 2 m was a good compromise between including information of too many surrounding trees and not accounting for the geometric offset at all.
...
Such limitations and uncertainties apply to most applied studies and cannot be completely avoided.
...
To account for this and the high-dimensionality in general, studies used feature selection approaches like semi-supervised feature extraction \cite{xia2015}, wrapper methods \cite{fassnacht2014, feng2016}, PCA and adjusted feature selection \cite{rochac2016}.
We could not find a study which made use of filter methods in combination with hyperparameter tuning in the field of (hyperspectral) remote sensing.
...
% RS studies with more/other algs than RF
Studies which include more than one model usually operate in a low-dimensional setting 
\cite{xu2019}.
...
In contrast, dataset VI showed a substantially poorer performance than the dataset with the original features (HR).
...
The potential of predicting defoliation for single trees as conducted in this study was rather limited with an average RMSE of 27 p.p..
We suggest that training data covering a wider range of defoliation values in a larger number of forest plantations would further improve the proposed models and result in more generalizable monitoring algorithms.
