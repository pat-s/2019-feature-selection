...
One aspect of this research field is to enhance the understanding of biotic and abiotic triggers, for example by analyzing tree defoliation \cite{hawrylo2018}.
...
The RF hyperparameter \texttt{\(m_{try}\)} was re-expressed as $m_try = p^{x_try}$.
The exponent $x_try$, (ranging from 1 to $p_sel$, where $p_sel$ is the number of features selected with the corresponding filter settings), was optimized between 0 (one randomly selected feature) and 0.5 ($\sqrt{p_sel}$ randomly selected candidate features).
...
In contrast, XGBoost and SVM used in all cases but Laukiz1 (less than 50 features) more than two-thirds of all available features.
...
Realizing early that few features are needed during tuning to reach adequate performances can reduce the overall computational runtime substantially, especially when iterating on features like \texttt{\(m_{try}\)} which are dependent on the number of features.
...
Potential reasons for this gap could be an easier access to wrappers methods and a higher general awareness of such compared to filter methods.
...
%	[]{Patrick Schratz}
	is a Data Scientist currently working in Zurich, CH.
    His area of expertise is applied machine learning, more specifically the field of environmental modeling.
    He is also a PhD candidate at the GIScience group, Department of Geography at University of Jena where he acts as a researcher in the environmental modeling field.
    At cynkra GmbH, Patrick is an R consultant with many years of experience in the following fields: CI/CD, package development, DevOps tasks, machine learning and spatio-temporal data handling.
\end{IEEEbiography}
\begin{IEEEbiography}
	[]{Jannes Muenchow}
	Biography text here.
