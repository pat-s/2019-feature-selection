...
pred <- bm_models_vi_task_lasso_no_filter_mbo$results$`defoliation-all-plots-VI`$`Lasso-MBO`$pred
...
loadd(bm_models_vi_task_rf_no_filter_mbo)
pred_rf <- bm_models_vi_task_rf_no_filter_mbo$results$`defoliation-all-plots-VI`$RF$pred
...
# without outlier
resid_rf1 <- resid_rf
resid_rf1$fitted.values[1526] <- NA

par(mfrow = c(1, 2))
plot(resid_rf1, which = c(1:2), ask = FALSE, main = "No Transform")
par(mfrow = c(1, 1))
```
