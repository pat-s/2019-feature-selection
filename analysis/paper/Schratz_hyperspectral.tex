...
\noindent To use the full information from the hyperspectral data, we calculated all possible vegetation indices supported by the \textit{hsdar} package (90 in total) as well as all possible \ac{NRI} combinations.
NRIs were calculated using the following formula:
...
Besides the well-known \ac{SVM} algorithm \citep{vapnik1998}, we also used \textit{xgboost} which is ensemble method relying on the idea of tree boosting that gained a lot of attention in recent years \citep{chen2016}.
