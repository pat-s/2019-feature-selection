...
```{r}
df_perf <- getBMRPerformances(bm_aggregated_new, as.df = T) %>%
...
saveRDS(table2, "docs/00-manuscripts/ieee/presentation/table-best-learner-per-task.rda")
...
results_aggr2 %>%
  ...
  facet_wrap(~task.id) +
  scale_color_manual(guide = guide_legend(override.aes = list(shape = c(19, 3))),
                     values = c("Filter" = "#BC8C29FF",
