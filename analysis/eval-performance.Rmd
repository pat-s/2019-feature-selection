...
df_tbl <- getBMRTuneResults(bmr_inspect_tune, as.df = TRUE) %>%
  ...
  dplyr::select(Learner, Plot, "Features (\\%)", RMSE)
...
df_tbl[[1]][first] <-
  paste0("\\midrule\\multirow{", rle.lengths, "}{*}{\\textbf{", df_tbl[[1]][first], "}}")
table4 <- df_tbl %>%
  xtable::xtable(
    type = "latex",
    caption = "Optimized number of features for selected learner-filter settings across folds for task HR-NRI-VI.",
