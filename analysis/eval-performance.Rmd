...
df_tbl <- getBMRTuneResults(bmr_inspect_tune, as.df = TRUE) %>%
  ...
  dplyr::mutate("\\#" = case_when(
    `Test Plot` == "Laukiz1" ~ ceiling((`Features (\\%)` * 559) / 100),
    `Test Plot` == "Laukiz2" ~ ceiling((`Features (\\%)` * 451) / 100),
    `Test Plot` == "Luiando" ~ ceiling((`Features (\\%)` * 301) / 100),
    `Test Plot` == "Oiartzun" ~ ceiling((`Features (\\%)` * 497) / 100),
  )) %>%
