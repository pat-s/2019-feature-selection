...
all_plots = readRDS("/data/patrick/mod/hyperspectral/spcv/all_plots_xgboost_blocking.rda")
all_plots_7vars = readRDS("/data/patrick/mod/hyperspectral/spcv/all_plots_xgboost_blocking_7vars.rda")
...
df <- tibble(
  ...
  rmse = c(mean(results_reps$laukiz1$rmse), mean(results_reps$laukiz2$rmse), mean(results_reps$luiando$rmse), mean(results_reps$oiartzun$rmse)),
...
## Differences between models (single vs super)

```{r}
# laukiz1
all_plots$measures.test$rmse[3] - mean(results_reps$laukiz1$rmse)

# laukiz2
all_plots$measures.test$rmse[4] - mean(results_reps$laukiz2$rmse)

# luiando
all_plots$measures.test$rmse[2] - mean(results_reps$luiando$rmse)

# oiartzun
all_plots$measures.test$rmse[1] - mean(results_reps$oiartzun$rmse)
```
...
```{r}
prow = plot_grid(rmse_dens, rmse_defol_cov,  align = 'vh', nrow = 1)
plots_grid <- plot_grid(prow, legend, rel_widths = c(2, .5))
```
...
```{r}
save_plot(here("03_figures/results/plot-characteristics.png"), plots_grid, ncol = 3)
```
