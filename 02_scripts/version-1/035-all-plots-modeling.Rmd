...
```{r}
# 5h 30 min with 2 nmc
var_imp = generateFeatureImportanceData(task, learner = lrn_xgboost, 
                                        measure = rmse, nmc = 50)
var_imp = readRDS("/data/patrick/mod/hyperspectral/variable-importance/bf2_ncm50rda")
...
```{r}
m = readRDS("/data/patrick/mod/hyperspectral/prediction/xgboost_trained_tuned.rda")
...
```{r}
model_xgboost_defol = readRDS("/data/patrick/mod/hyperspectral/prediction/xgboost_trained_tuned.rda")
library("doParallel")
...
X = X[model_xgboost_defol$learner.model$feature_names]

predictor = Predictor$new(model_xgboost_defol, data = X, y = task$env$data$defoliation)
imp = FeatureImp$new(predictor, loss = "rmse", parallel = TRUE, n.repetitions = 100)
stopCluster(cl)
saveRDS(imp, "/data/patrick/mod/hyperspectral/variable-importance/feature-imp-iml.rda")

imp = readRDS("/data/patrick/mod/hyperspectral/variable-importance/feature-imp-iml.rda")
```
...
parallelStart(mode = "multicore", cpus = 4, level = "mlr.resample",
              mc.set.seed = TRUE)
set.seed(12345)
resa_svm <- mlr::resample(tune_wrapper, task_7,
                          resampling = outer, extract = getTuneResult,
                          show.info = TRUE, measures = list(rmse))
parallelStop()
saveRDS(resa_svm, "/data/patrick/mod/hyperspectral/spcv/all_plots_xgboost_blocking_7vars.rda")
```
...
```{r}
lrn_xgboost = makeFilterWrapper(learner = lrn_xgboost,
                                imp.learner = c("regr.xgboost"))
```
...
```{r}
tune_wrapper_mrmr <- makeTuneWrapper(lrn_xgboost, resampling = inner, 
                                     par.set = ps_xgboost_mrmr,
                                     control = tune.ctrl, show.info = TRUE,
                                     measures = list(rmse))
tune_wrapper_permut <- makeTuneWrapper(lrn_xgboost, resampling = inner, 
...
#               mc.set.seed = TRUE)
set.seed(12345)
resa_svm <- mlr::resample(tune_wrapper, task, models = FALSE,
                          resampling = outer, 
                          extract = function(x) list(getFeatSelResult, getTuneResult),
                          show.info = TRUE, measures = list(rmse))

benchm <- benchmark(tune_wrapper, task, models = FALSE,
                          resampling = outer, 
                          extract = function(x) list(getFeatSelResult, getTuneResult),
                          show.info = TRUE, measures = list(rmse))

parallelStop()
saveRDS(resa_svm, "/data/patrick/mod/hyperspectral/spcv/all_plots_xgboost_fs_tune.rda")
```
