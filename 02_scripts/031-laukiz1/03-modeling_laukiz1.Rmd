...
```{r}
ps_glmnet <- makeParamSet(makeNumericParam("s", lower = 77, upper = 7700))
```
...
parallelStop()
saveRDS(resa_glmnet, "/data/patrick/mod/hyperspectral/spcv/laukiz1_ridge.rda")
```
...
```{r}
ps_glmnet <- makeParamSet(makeNumericParam("s", lower = 0.07, upper = 7.7))
```
...
parallelStop()
saveRDS(resa_glmnet, "/data/patrick/mod/hyperspectral/spcv/laukiz1_lasso.rda")
```
...
```{r}
ps_glmnet <- makeParamSet(makeNumericParam("s", lower = 0.07, upper = 7700),
...
parallelStop()
saveRDS(resa_glmnet, "/data/patrick/mod/hyperspectral/spcv/laukiz1_elasticnet")
```
