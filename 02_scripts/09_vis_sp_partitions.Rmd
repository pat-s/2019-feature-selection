...
Visualize partitions for all four plots and then take only fold 1 of each.

```{r}

all = imap(c("laukiz1", "laukiz2", "luiando", "oiartzun"), ~ {
  data = readRDS("/data/patrick/mod/hyperspectral/data_clean_with_indices/data_clean_standardized_bf2.rda")
  coords = readRDS("/data/patrick/mod/hyperspectral/data_clean_with_indices/data_clean_standardized_bf2-coords.rda")
  ...
  task = makeRegrTask(id = "data", data = data, 
                      target = "defoliation", coordinates = coords)
  
  resamp_sp <- readRDS(glue::glue("/data/patrick/mod/hyperspectral/spcv/{.x}_xgboost.rda"))
  
  plist <- createSpatialResamplingPlots(task, list("SpCV" = resamp_sp),
                                        crs = 32630, repetitions = 1, point.size = 1.5,
                                        y.axis.breaks = c(43.3541, 43.353, 43.2648, 43.0955), 
                                        x.axis.breaks = c(-2.93, -2.9293, -2.99577,
                                                          -1.845),
                                        axis.text.size = 18)
})

plots = plot_grid(all[[1]][["Plots"]][[1]],
                  all[[2]][["Plots"]][[1]],
                  all[[3]][["Plots"]][[1]],
                  all[[4]][["Plots"]][[1]],
                  labels = c("Fold 1 (Laukiz 1)", "Fold 1 (Laukiz 2)",
                             "Fold 3 (Luiando)", "Fold 4 (Oiartzun)"),
                  nrow = 1, ncol = 4)

save_plot(plots, filename = "../03_figures/data/spcv_folds_demon_plots.png",
          ncol = 4, nrow = 1)
```
...
library(furrr)
library(magick)
imgs = list.files("/home/patrick/PhD/papers/03_hyperspectral/03_figures/data", 
