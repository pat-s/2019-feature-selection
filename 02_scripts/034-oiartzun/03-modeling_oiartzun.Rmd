...
```{r}
ps_glmnet <- makeParamSet(makeNumericParam("s", lower = 55, upper = 5525))
```
...
```{r}
tibble(index = rownames(as.matrix(coef(glmnet_train_tuned$learner.model, s = 5460))), 
       coef = as.vector(as.matrix(coef(glmnet_train_tuned$learner.model, s = 5460)))) %>% 
  arrange(desc(coef))
```
...
```{r}
ps_glmnet <- makeParamSet(makeNumericParam("s", lower = 0.05, upper = 5.5))
```
...
```{r}
ps_glmnet <- makeParamSet(makeNumericParam("s", lower = 0.05, upper = 5525),
