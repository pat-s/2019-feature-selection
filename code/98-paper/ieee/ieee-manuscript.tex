...
This simplifies the analysis of (temporal) changes of the environment on a large scale \cite{ma2015}.
...
\subsection{Predictive performance - general}

% - results across tasks show no substantial difference
% - RMSE around 34
...
A noticable difference between SVM and all other learners was visible in all feature sets without NRI variables (difference around 3-4 RMSE).

In all tasks with NRI variables, the difference was either relatively small (0.2, HR-NRI-VI) or another learner achieved a better result (NRI, Ridge-CV).
Overall, the response variable defoliation at trees could be modeled with an error of 34 \%.
Learner RF in combination with a Carscore filter scored the absolute lowest error on the NRI task with a RMSE of 3.396.

L1 penalized regression showed slightly better results in three out of six tasks when being optimized using the package internal 10-fold CV (50 vs 58 RMSE).
Regression with Ridge penalty showed a high variance when comparing results across tasks: In three out of six tasks (all including VI variables) the error was too high to be visualized (HR-NRI-VI; HR-VI, VI), for HR-NRI and HR the error was medium (around 40 RMSE), and for task NRI Ridge regression achieved almost the same score as the best performing model (3.39 RMSE, NRI).
All penalized methods which were tuned using MBO show exactly the same error across all tasks.
