...
aoi <- download_aoi("https://zenodo.org/record/3476044/files/aoi.gpkg")
...
images_stack <- map2(list(records_2017, records_2018), images_unzip, ~ stack_bands(.x, .y))

# copy cloud covers from unziped files ------------------------------
cloud_stack <- future_map2(list(records_2017, records_2018), images_unzip, ~ copy_cloud(.x, .y))

# mosaic cloud covers of same date ------------------------------
cloud_mosaic <- future_map2(list(records_2017, records_2018), cloud_stack, ~ mosaic_clouds(.x, .y))

# mosaic images of the same date ------------------------------
mosaic <- future_map2(list(records_2017, records_2018), images_stack, ~ mosaic_images(.x, .y))

# mask mosaics ------------------------------
mosaic_masked <- future_map2(mosaic, cloud_mosaic, ~ mask_mosaic(.x, .y, aoi, forest_mask))

# calculate vegetation indices ------------------------------
mosaic_vi <- map(mosaic_masked, ~ calculate_vi(.x))

# create raster objects of vegetation indices ------------------------------
ras_veg_inds <- future_map(mosaic_vi, ~ mask_vi(.x))

# create sf objects of vegetation indices ------------------------------
sf_veg_inds <- map(ras_veg_inds, ~ ras_to_sf(.x))

# get coordinates from sf object for vegetation indices ------------------------------
coordinates <- future_map(sf_veg_inds, ~ get_coordinates(.x))

# create prediction data ------------------------------
prediction_df <- future_map(sf_veg_inds, ~ create_prediction_df(.x, model = train_xgboost))
...
defoliation_df_relative <- future_map(defoliation_df, ~ scale_defoliation(.x))
