...
Using only the seven most important variables (\autoref{fig:var-imp}) shows small increases in performance for Laukiz 1 and Laukiz 2, a small decrease for Luiando and almost a reduction of 50\% of the error for Oiartzun (32.97 vs. 17.62 RMSE).
...
One example here is tree age: The older a tree the more vunerable it may be to pathogen infections causing defoliation.
However, such predictors would not be available for a spatial prediction scenario and one of the main goals of this study is to relate defoliation to variables that are available on a larger scale (e.g. remote sensing indices).
	...
	[COMPARE ALL VARIABLES VS 7 VARIABLES]
...
Even though we are not specifically looking at vegetation health but at the level of defoliation, these indices seem to help the model deciding whether a tree is defoliated or not.
The vegetation indeces could help here in two ways:
1) Trees that show a high level of defoliation do also reflect their bad health status through the remaining foliation.
2) Defoliated trees have more influence of bare ground information in their pixel values and will therefore be identified as defoliated by the model.

Even though no NRI made it among the most important variables in this study (stating that the first seven of this study are the most important ones), it is interesting that they occupy all ranks from 8 - 30 (\autoref{fig:var-imp}).
...
All indices starting from rank eight do simply have a very small score.
Nevertheless, one could also make the cut at rank five here.
However, the break between vegetation indeces and NRI at rank eight also had an influence in setting this subjective break there.
